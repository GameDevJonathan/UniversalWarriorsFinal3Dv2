    ...
    public class SetActiveAction : CutSceneAction
    {
        ...
        public override void CustomEditor(CutSceneNode cutSceneNode, CutsceneGraph graph, List<CutSceneNode> selectedNodes = null)
        {
            ...
            if (node.objectSource == ObjectSource.AssignObject)
                setActiveObject = (GameObject)EditorGUILayout.ObjectField("GameObject", node.setActiveObject, typeof(GameObject), true);
            else
            {
                GUILayout.Label("GameObject");
                GUILayout.Label(new GUIContent("GameObject", "Player-tagged object"));
            }
            ...
            //var setActiveObject = (GameObject)EditorGUILayout.ObjectField("GameObject", node.setActiveObject, typeof(GameObject), true);
            //if (setActiveObject != node.setActiveObject)
            //{
            //    UndoGraph(graph);
            //    foreach (var n in actions)
            //        n.setActiveObject = setActiveObject;
            //}
            var objectState = (ActivationState)EditorGUILayout.EnumPopup(node.objectState, GUILayout.Width(65));
            ...
            ValidationWarning(node.setActiveObject, "Object is not assigned", graph);
        }
